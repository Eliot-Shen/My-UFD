...
class RealFakeDataset(Dataset):
    def __init__(self, opt):
        ...
        if opt.data_mode == 'ours':
            ...
            postfix = "train" if opt.data_label=="train" else "val" 
            real_list = get_list( os.path.join(opt.real_list_path, postfix) )  
            fake_list = get_list( os.path.join(opt.fake_list_path, postfix) )
            # dataset format:  /real/train  /real/val  /fake/train  /fake/val
        ...
        if opt.isTrain and not opt.no_crop:
            print("Use RandomCrop.")
            # crop_func = transforms.RandomCrop(opt.cropSize)
        elif opt.no_crop:
            print("Not use crop.")
            crop_func = transforms.Lambda(lambda img: img)
        # else:
        #     print("Use CenterCrop")
        #     crop_func = transforms.CenterCrop(opt.cropSize)

        if opt.isTrain and not opt.no_flip:
            print("Use RandomHorizontalFlip.")
            flip_func = transforms.RandomHorizontalFlip()
        else:
            print("Not use RandomHorizontalFlip.")
            flip_func = transforms.Lambda(lambda img: img)
        # if not opt.isTrain and not opt.no_resize:
        #     # rz_func = transforms.Lambda(lambda img: img)
        #     print("Resize image to (224,224)")
        #     rz_func = transforms.Resize([224,224])
        # else:
        print("Resize image to ({0},{0})".format(opt.loadSize)," using ", opt.rz_interp," method.")
        # rz_func = transforms.Resize([224,224])
        rz_func = transforms.Resize([336,336])

        print("Use cut func")
        # rz_func = transforms.Lambda(lambda img: custom_resize(img, opt))

        if opt.isTrain and opt.data_aug:
            print("Using blur and jpeg augment.")
            data_aug_func = transforms.Lambda(lambda img: data_augment(img, opt))
        else:
            print("Do not use blur and jpeg augment.")
            data_aug_func = transforms.Lambda(lambda img: img)
...
def data_augment(img, opt):
    # print("Using blur and jpeg augment.")
    # print("Blur prob:", opt.blur_prob)
    # print("Jpeg prob:", opt.jpg_prob)
    img = np.array(img)
    if img.ndim == 2:
